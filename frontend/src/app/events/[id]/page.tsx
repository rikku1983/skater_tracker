...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const eventRows = await sql`
    ...
    GROUP BY e.id
  `
  if (!eventRows[0]) notFound()
  const event = eventRows[0] as { id: number; event_name: string; season: string; start_date: string | null; venue: string | null; city: string | null; state: string | null; result_count: number }

  const [results, classification, timeClass] = await Promise.all([
    sql`
      ...
      WHERE r.event_id = ${eid} AND r.is_relay = false
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Badge variant="outline">{Number(event.result_count).toLocaleString()} results</Badge>
        </div>
      </div>
      ...
      <Tabs defaultValue="results">
        ...
        <TabsContent value="results" className="mt-4">
          <EventResultsTable rows={results as Record<string, unknown>[]} />
        </TabsContent>
        <TabsContent value="classification" className="mt-4">
          <EventClassificationTable rows={classification as Record<string, unknown>[]} />
        </TabsContent>
        <TabsContent value="time_class" className="mt-4">
          <EventTimeClassTable rows={timeClass as Record<string, unknown>[]} />
        </TabsContent>
      </Tabs>
    </div>
  )
